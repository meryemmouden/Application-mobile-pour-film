...
class FilmDetail extends React.Component {
  ...
  async componentDidMount() {
    try {
      const data = await getFilmDetailFromApi(this.props.route.params.filmId);
      this.setState({
        film: data,
        isLoading: false
      });
    ...
    if (film != undefined) {
      return (
        <ScrollView style={styles.scrollview_container}>
          ...
          <Text style={styles.default_text}>Sorti le {dayjs(film.release_date).format('DD/MM/YYYY')}</Text>
